...
# ___________________________________________________________________
    ...
    shape_functions_array_beam = {
            2: lambda x,l: np.array([
                                    ( 1-( (3*np.pow(x,2)) / np.pow(l,2) )+((2*np.pow(x,3))/np.pow(l,3)) ),# v_1
                                    (x-((2*np.pow(x,2))/l)+(np.pow(x,3)/np.pow(l,2))), #\phi_1
                                    (((3*np.pow(x,2))/np.pow(l,2))+((2*np.pow(x,3))/np.pow(l,3))),# v_2
                                    (-((2*np.pow(x,2))/l)+(np.pow(x,3)/np.pow(l,2))) #\phi_2
                                    ]),
        }
    ...
    strain_dispacement_array_beam = {
            2: lambda x,l: np.array([
                                    ((12*x/np.pow(l,3))-(6/np.pow(l,2))),
                                    ((6*x/np.pow(l,2))-(4/l)),
                                    ((6*x/np.pow(l,2))-(12*x/np.pow(l,3))),
                                    ((6*x/np.pow(l,2))-(2/l))
                                    ]),
        }
# ___________________________________________________________________
...
# ___________________________________________________________________
    ...
    def __calc_k_local(self,element_index):
        ...
        # Gauss quadrature points and weights
        # 2 points exact for linear, 3 points for quadratic
        #TODO find a way to calculate degrees for other SF
        #degree_BtB = 3 * (n_nodes - 2)
        # n_gauss = int(np.ceil((degree_BtB + 1) / 2))
        # n_gauss = max(n_gauss, 1)  # at least 1 point
        n_gauss = 3
        ...
        #EXTRA PASS FOR EULER BERNOULLI
        N_func=np.zeros((2,n_nodes*3))
        for j in range(n_nodes):
            N_func[0][j*3]=self.shape_functions_array_bar[j]
            for i in range(1,3):
                N_func[1][j*3+i]=self.shape_functions_array_beam[j*2+i]
        # integrate K_local = EA * ∫ Bᵀ B (L/2) dξ
        K_local = np.zeros((n_nodes, n_nodes))
        B=np.zeros((2,n_nodes*3))
        for i in range(2):
            for xi, w in zip(points, weights):
                B[i] = N_func[i](xi, L)                        # (n_nodes,)
                K_local += w * np.outer(B, B) * (L/2)
        #K_local *= self.E[element_index] * self.A[element_index] 
